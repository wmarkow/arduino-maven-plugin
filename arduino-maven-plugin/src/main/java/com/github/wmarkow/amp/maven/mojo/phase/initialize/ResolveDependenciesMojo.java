...
public class ResolveDependenciesMojo extends GenericPlatformMojo
{
    ...
    public void execute() throws MojoExecutionException, MojoFailureException
    {
        ...
        for (Artifact arduinoLib : getMissingArduinoDependencies())
        {
            if (ARDUINO_CORE_EXTENSION.equals(arduinoLib.getExtension()))
            {
                ...
                try
                {
                    ArduinoCoreArtifactResolver fetcher = new ArduinoCoreArtifactResolver(getPlatformPackageManager());
                    File fetchedCore = fetcher.fetch(arduinoLib.getArtifactId(), arduinoLib.getVersion(), workDir);
            ...
            {
                // a special case: install an empty zip file as this arduino library (just to
                // ignor Maven
                // errors about missing dependency). This library will be handled in a different
                // way later.
                try
                {
    ...
    private void installLibrary(Artifact arduinoLib, File libraryFile) throws IOException, InstallationException
    {
    ...
    private List<Artifact> collectDependencies()
    {
        ...
        if (ARDUINO_LIB_EXTENSION.equals(projectArtifact.getExtension()))
        {
            ...
            DependencySelector dependencySelector = new AndDependencySelector(
                    new ScopeDependencySelector(JavaScopes.TEST), new OptionalDependencySelector());
            session.setDependencySelector(dependencySelector);
        }

        org.eclipse.aether.graph.Dependency dep = new org.eclipse.aether.graph.Dependency(projectArtifact, null);
